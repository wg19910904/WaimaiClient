...
-(void)reloadCellWithArr:(NSArray *)kindItemArr{
    [self.kindItemCollection mas_updateConstraints:^(MASConstraintMaker *make) {
        make.height.offset = kindItemArr.count >= 5 ? 205*KUI_SCALE : 96;
        make.left.right.top.offset = 0;
        make.bottom.offset = 0;
    }];
...
- (void)setDataDic:(NSDictionary *)dataDic{
    ...
    if (textColor.length) {
        self.kindItemCollection.textColor = textColor;
    }else{
        self.kindItemCollection.textColor = nil;
    }
    if(dataArr.count){
