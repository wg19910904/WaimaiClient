...
JSExportAs(onPaymentByCode, -(void)onPaymentByCode:(id)obj);
...
-(void)goHome{
    if (self.jsDelegate && [self.jsDelegate respondsToSelector:@selector(goHome)]) {
        [self.jsDelegate goHome];
    }
}
#pragma mark -获取坐标
-(void)getLocation{
...
#pragma mark -直接调用微信支付
-(void)onPaymentByCode:(id)obj{
    NSDictionary *dic;
    if ([obj isKindOfClass:[NSDictionary class]]) {
        dic = obj;
    }else{
    ...
    if (self.jsDelegate && [self.jsDelegate respondsToSelector:@selector(onPaymentByCode:)]) {
        [self.jsDelegate onPaymentByCode:dic];
    }
}
...
- (void)showBackBtnAndTitle:(id)obj {
    ...
    if (self.jsDelegate && [self.jsDelegate respondsToSelector:@selector(showBackBtnAndTitle:)]) {
        [self.jsDelegate showBackBtnAndTitle:dic];
    }
}
...
+(id)jumpWithLink:(NSString *)link{
    ...
    if ([link containsString:[NSString stringWithFormat:@"waimai.%@",KReplace_Url]]||IsWaiMai) {
        ...
        if ([link containsString:@"ucenter/member.html"]) {
            ...
            vc = [NSClassFromString(@"JHNewWaiMaiMySelfVC") new];
            goto back;
        }
