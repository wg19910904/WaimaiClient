...
#import "JHUserAccountSetRevisePsdCell.h"
#import <IQKeyboardManager.h>
#import "JHUserAccountSetViewModel.h"
#import "JHWaimaiMineViewModel.h"
@interface JHUserAccountSetRevisePsdVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
...
@implementation JHUserAccountSetRevisePsdVC
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString *str = @"JHUserAccountSetRevisePsdCell";
    JHUserAccountSetRevisePsdCell *cell = [tableView dequeueReusableCellWithIdentifier:str];
    if (!cell) {
        cell = [[JHUserAccountSetRevisePsdCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:str];
    }
    cell.superVC = self;
...
-(void)clickSubmitBtn{
    ...
    if (str) {
        ...
        [self postToRevisePsd];
    }
}
#pragma mark - 修改密码的接口
-(void)postToRevisePsd{
    SHOW_HUD
    [JHUserAccountSetViewModel postToRevisePsdWithDic:@{@"new_passwd":_psdField.text,@"sms_code":_securityField.text} block:^(NSString *error) {
        HIDE_HUD
        if (error) {
