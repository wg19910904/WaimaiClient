...
@interface ZQMapView()<MKMapViewDelegate>{
    CGPoint annotationPoint;//大头针显示的中心点
    BOOL isFirstShow;//是否是第一次展示大头针
}
@property(nonatomic,strong)MKMapView *mapView;//地图
@property(nonatomic,strong)ZQAnnotation *annotation;//大头针
@property(nonatomic,strong)NSString *title;//大头针的标题
@property(nonatomic,strong)NSString *subTitle;//大头针子标题
@property(nonatomic,strong)MKAnnotationView * annotationV;//展示大头针的view
...
-(MKMapView *)mapView{
    if (!_mapView) {
        ...
        annotationPoint = CGPointMake(_annotationV.centerX, _annotationV.centerY+40);
        [self annotation];
        isFirstShow = YES;
    }
    return _mapView;
}
#pragma mark - 改变地图的缩放级别的
-(void)spanWithMapViewWithSpan:(MKCoordinateSpan)span{
    if (_annotationV) {
        [_annotationV removeFromSuperview];
        _annotationV = nil;
        [_mapView removeAnnotation:_annotation];
        _annotation = nil;
    }
    [self annotation];
    MKCoordinateRegion regoin = MKCoordinateRegionMake(_coordinate_annotation, span);
    [_mapView setRegion:regoin animated:YES];
    [_mapView setCenterCoordinate:_coordinate_annotation animated:YES];
}
#pragma mark - 添加大头症
-(ZQAnnotation *)annotation{
    if (!_annotation) {
        _annotation = [[ZQAnnotation alloc]init];
...
-(MKAnnotationView *)mapView:(MKMapView *)mapView viewForAnnotation:(id<MKAnnotation>)annotation{
    if (!_annotationV) {
        _annotationV = [[MKAnnotationView alloc]init];
        _annotationV.image = self.annotationImage?self.annotationImage:[UIImage imageNamed:@"icon_qishou"];
        _annotationV.centerOffset =  annotationPoint;
        _annotationV.canShowCallout = YES;
        return _annotationV;
...
-(void)changePraviteWithPeiCoordinate:(CLLocationCoordinate2D)peiCoordinate
    ...
    [self spanWithMapViewWithSpan:MKCoordinateSpanMake(0.01, 0.01)];
    _title = mainT;
    _subTitle = subT;
    _annotation.title = _title;
    _annotation.subtitle = _subTitle;
...
-(void)showAnnotationViewLat:(NSString *)lat
    ...
    [self spanWithMapViewWithSpan:MKCoordinateSpanMake(0.03, 0.03)];
