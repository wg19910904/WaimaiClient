...
@interface ZQClassView()<UITableViewDelegate,UITableViewDataSource>{
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (_isShangCheng && !_isGoods) {
        ...
        subOldModel = model;
        [_rightTab reloadData];
        [ZQSaveSelecter saveModel].isClass = YES;
        [ZQSaveSelecter saveModel].isSub = YES;
        [ZQSaveSelecter saveModel].indexP = [NSIndexPath indexPathForRow:indexPath.row inSection:0];
        if (self.removeBlock) {
            self.removeBlock();
        }
        ...
        if (tableView == _leftTab) {
            ...
            if (!model.subArr||model.subArr.count == 0) {
                [ZQSaveSelecter saveModel].isClass = YES;
                [ZQSaveSelecter saveModel].indexP = [NSIndexPath indexPathForRow:0 inSection:indexPath.row];
                if (self.removeBlock) {
                    self.removeBlock();
                }
            }else{
            ...
            [ZQSaveSelecter saveModel].indexP = [NSIndexPath indexPathForRow:indexPath.row inSection:sec];
            if (self.removeBlock) {
                self.removeBlock();
            }
        }
