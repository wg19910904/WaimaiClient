...
- (void)addAnnotation:(CLLocationCoordinate2D)point
    ...
    if ([XHMapKitManager shareManager].is_International) {
        ...
        marker.title = title;
        marker.map = _gmsMap;
        if (imgStr.length > 0) marker.icon = [UIImage imageNamed:imgStr];
...
#pragma mark - 改变客户和商家之间的距离的
-(void)changeDistanceWithShopCoordinate:(CLLocationCoordinate2D)shopCoordinate
                          customCoordinate:(CLLocationCoordinate2D)customCoordinate{
    CLLocationCoordinate2D shop = [self changeBaiDuToGaodeWithBaidu:shopCoordinate];
    CLLocationCoordinate2D custom = [self changeBaiDuToGaodeWithBaidu:customCoordinate];
    NSString *dis = [self getDistanceWithPoint1:shop point2:custom];
    NSString *sub = [NSString stringWithFormat:NSLocalizedString(@"距商家有%@", nil),dis];
    [self changePraviteWithCustomCoordinate:custom subT:sub mainT:@""];
}
...
/**
 ...
 @param mainT 大头针主标题
 */
-(void)changePraviteWithCustomCoordinate:(CLLocationCoordinate2D)customCoordinate
                                 subT:(NSString *)subT
                                mainT:(NSString *)mainT{
    if ([XHMapKitManager shareManager].is_International) {
        //国际客户
        ...
        //国内客户
        [_gaodeMap removeAnnotations:_gaodeMap.annotations];
        _gaodeMap.showsUserLocation = NO;
    }
    [self addAnnotation:customCoordinate
                  title:subT imgStr:@"icon_round"
               selected:YES];
}
...
- (void)getGMSMapNaviPointsWithOrigin_lat:(double)origin_lat
                          ...
                          destination_lat:(double)destination_lat
                          destination_lng:(double)destination_lng{
    __block GMSMutablePath *path;
    //添加起点
    //    [path addLatitude:[XHMapKitManager shareManager].lat
    //            longitude:[XHMapKitManager shareManager].lng];
    [path addLatitude:origin_lat
            longitude:origin_lng];
    //设置起点为中心点
    _gmsMap.camera = [GMSCameraPosition cameraWithLatitude:[XHMapKitManager shareManager].lat
                                                 longitude:[XHMapKitManager shareManager].lng
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSString *url=[NSString stringWithFormat:@"https://maps.googleapis.com/maps/api/directions/json?origin=%f,%f&destination=%f,%f&sensor=false&mode=%@&key=%@",origin_lat,origin_lng,destination_lat,destination_lng,@"driving",GMS_MapKey];
        ...
        [NSURLConnection sendAsynchronousRequest:request queue:queue completionHandler:^(NSURLResponse *response, NSData *data, NSError *connectionError) {
            if (connectionError == nil) {
                ...
                NSDictionary *tempDic = subDic[@"overview_polyline"];
                
                NSString *points = tempDic[@"points"];
                path = [self polylineWithEncodedString:points];
